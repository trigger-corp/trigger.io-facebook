...
public class API {
	...
	public static void authorize(final ForgeTask task, @ForgeParam("permissions") final JsonArray permissionsJSON, @ForgeParam("dialog") final boolean dialog) {
		...
		if (prefs.getString("permissions", "").equals(fpermissionsJSON)) {
			if (access_token != null) {
				getFacebook(task).setAccessToken(access_token);
			}
			if (expires != 0) {
				getFacebook(task).setAccessExpires(expires);
			}
		}

		if (getFacebook(task).isSessionValid()) {
			JsonObject details = new JsonObject();
			details.addProperty("access_token", getFacebook(task).getAccessToken());
			details.addProperty("access_expires", getFacebook(task).getAccessExpires());
			task.success(details);
		} else if (dialog) {
			task.performUI(new Runnable() {
				public void run() {
					getFacebook(task).authorize(ForgeApp.getActivity(), permissions, new DialogListener() {
						public void onComplete(Bundle values) {
							SharedPreferences.Editor editor = prefs.edit();
							editor.putString("access_token", getFacebook(task).getAccessToken());
							editor.putLong("access_expires", getFacebook(task).getAccessExpires());
							editor.putString("permissions", fpermissionsJSON);
							editor.commit();
							JsonObject details = new JsonObject();
							details.addProperty("access_token", getFacebook(task).getAccessToken());
							details.addProperty("access_expires", getFacebook(task).getAccessExpires());
